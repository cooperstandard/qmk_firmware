...
void thumbstick_init(void) {
    ...
    thumbstick_state.config.mode           = THUMBSTICK_MODE_ARROWS;
...
int16_t thumbstick_get_mouse_speed(int16_t component) {
    int16_t  maxSpeed;
    uint16_t distance = component < 0 ? (-1) * component : component;
...
thumbstick_direction_t thumbstick_get_discretized_direction(thumbstick_vector_t vector, float axisSeparation, bool eightAxis) {
    ...
    uint16_t               absX                = vector.x < 0 ? (-1) * vector.x : vector.x;
    uint16_t               absY                = vector.y < 0 ? (-1) * vector.y : vector.y;
    uint16_t               maxComponent        = (absX > absY) ? absX : absY;
    bool                   insideDeadZone      = (maxComponent <= THUMBSTICK_DEAD_ZONE);
    bool                   outsideDiagonalZone = (((absX - absY < 0 ? absY - absX : absX - absY) / (float)maxComponent) >= axisSeparation);
    if (insideDeadZone) {
